...
sys_gorilla_movement::
    ...
    ; Esto se ejecuta *independientemente* del estado, (excepto en INIT).
    ld a, [snake_flags]
    and MASK_STATE
...
call change_entity_group_acc ; <-- CORRECCIÓN 1: Usar ACELERACIÓN

; Comprobar si ha aterrizado
ld a, ENEMY_START_ENTITY_ID
ld d, GORILLA_SPRITES_SIZE
call check_ground_collision
jr nc, .check_wall_collisions ; <-- CORRECCIÓN 2: Si no hay acarreo (no-collision), salta a comprobar paredes
.landed:
; ¡Acaba de aterrizar!
ld a, 1
...
call change_entity_group_vel
    ...
    ; --- AÑADIR ESTO ---
    ; Parar también la aceleración (¡para que la gravedad no siga empujando!)
ld bc, $0000 ; Acc Y = 0
ld de, $0000 ; Acc X = 0
ld d, GORILLA_SPRITES_SIZE
...
.init_state:
    ...
    or STATE_IDLE_GORILLA ; Empezar en IDLE
    ld [snake_flags], a
...
.apply_jump:
    ld d, GORILLA_SPRITES_SIZE  ; <-- CORREGIDO
